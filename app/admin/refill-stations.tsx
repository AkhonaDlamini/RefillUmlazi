...
export default function RefillStations() {
  ...
  const locationCoordinates: { [key: string]: { latitude: number; longitude: number } } = {
    ...
    'Umlazi B': { latitude: -29.966, longitude: 30.927 },
    'Umlazi C': { latitude: -29.9522, longitude: 30.903 },
    'Umlazi D': { latitude: -29.9678, longitude: 30.903 },
    'Umlazi E': { latitude: -29.9483, longitude: 30.9159 },
    'Umlazi F': { latitude: -29.974, longitude: 30.893 },
    'Umlazi G': { latitude: -29.946, longitude: 30.8815 },
    'Umlazi H': { latitude: -29.949, longitude: 30.87 },
    'Umlazi I': { latitude: -29.9688, longitude: 30.8843 },
    'Umlazi J': { latitude: -29.952, longitude: 30.8585 },
  ...
  useEffect(() => {
    try {
      ...
      Alert.alert("Error", "Unable to listen for station updates.");
    }
  }, []);
  ...
  const addOrUpdateStation = async () => {
    ...
    try {
      ...
      await dbCall(dbRef, data);
      Alert.alert('Success', editingId ? 'Station updated' : 'Station added');
      resetFields();
    } catch (err: any) {
      console.error("Error saving station", err);
      Alert.alert("Error", err.message || "Could not save station.");
    }
  };
  ...
  const deleteStation = (id: string) => {
    Alert.alert('Delete Station', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await remove(ref(db, `admin/refill-stations/${id}`));
            if (editingId === id) resetFields();
          } catch (err: any) {
            console.error("Failed to delete station", err);
            Alert.alert("Error", err.message || "Could not delete station.");
          }
        },
      },
    ]);
  ...
  const editStation = (s: RefillStation) => {
    try {
      ...
      console.error("Error loading station for editing", err);
  ...
  const handleMapPress = async (e: MapPressEvent) => {
    ...
    try {
      ...
      Alert.alert("Geocoding Failed", "Could not retrieve address from coordinates.");
    }
  };
